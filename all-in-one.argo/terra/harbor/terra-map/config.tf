resource "harbor_config_auth" "oidc" {
  ...
  oidc_client_secret = data.vault_kv_secret_v2.harbor_auth.data.AUTHENTIK_CLIENT_SECRET //base64encode(data.vault_kv_secret_v2.harbor_auth.data.AUTHENTIK_CLIENT_SECRET)
